	...
	real(8), dimension(nx1,ny,nz) :: uint, vaux
	real(8), dimension(nx,ny1,nz) :: vint, uaux
	...
	real(8), dimension(nx1,ny,nz) :: fundox
	real(8), dimension(nx,ny1,nz) :: fundoy
	real(8), dimension(nx,ny,nz)   :: velaux
	!contadores
	integer :: i, j, k
	integer :: ntal
	real(8)    :: tal, ur1, ur0
...
!	call interpz_cf(u(1:nx1,1:ny,1:nz),nx1,ny,nz,uz(1:nx1,1:ny,1:nz1)) !(nx1,ny,nz1)
 ...
 call interpy_fc(v(1:nx,1:ny1,1:nz),nx,ny1,nz,velaux)
 call interpx_cf(velaux,nx,ny,nz,vaux)
 
 call interpx_fc(u(1:nx1,1:ny,1:nz),nx1,ny,nz,velaux)
 call interpy_cf(velaux,nx,ny,nz,uaux)
...
!      vz(:,:,0)=vz(:,:,1)
	...
	do j = 1, ny
	do i = 1, nx1
		ur1 = sqrt(u(i,j,k+1)*u(i,j,k+1) + vaux(i,j,k+1)*vaux(i,j,k+1))
		ur0 = sqrt(u(i,j,k)*u(i,j,k) + vaux(i,j,k)*vaux(i,j,k))
		fundox(i,j,k) = gz/(chezy*chezy)*(ur1*u(i,j,k+1)-ur0*u(i,j,k))/dz
	enddo
	enddo
	enddo
	...
	do j=1, ny1
	do i=1, nx
		ur1 = sqrt(v(i,j,k+1)*v(i,j,k+1) + uaux(i,j,k+1)*uaux(i,j,k+1))
		ur0 = sqrt(v(i,j,k)*v(i,j,k) + uaux(i,j,k)*uaux(i,j,k))
		fundoy(i,j,k) = gz/(chezy*chezy)*(ur1*v(i,j,k+1)-ur0*v(i,j,k))/dz
	enddo
	enddo
	enddo
	...
	do i = 1, nx1
		Fu(i,j,k) = -uint(i,j,k) + ((ama(i,j,k)*2.*(u(i+1,j,k)-u(i,j,k))-&
                            ...
                            dma(i,j,k+1)*((w(i,j,k+1)-w(i-1,j,k+1))/dx) - &
                            dma(i,j,k)*(&
                            (w(i,j,k)-w(i-1,j,k))/dx  ))/rhox(i,j,k) + gx &
	...
	do i = 1, nx
		...
		Fv(i,j,k) = -vint(i,j,k) + (amb(i+1,j,k)*((v(i+1,j,k)-v(i,j,k))/dx+(u(i+1,j,k)-u(i+1,j-1,k))/dy) - &
                            ...
                            dmb(i,j,k+1)*((w(i,j,k+1)-w(i,j-1,k+1))/dy ) - &
                            dmb(i,j,k)  *((w(i,j,k)  -w(i,j-1,k))/dy   ))/rhoy(i,j,k) &
			    -sigma*dhsdy(i,j,k) +tf_v(i,j,k) + fundoy(i,j,k)	                
