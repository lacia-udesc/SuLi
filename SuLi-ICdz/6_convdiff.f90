...
!      vz(:,:,0)=vz(:,:,1)
	...
	call interpz_cf(xmu,nx1,ny,nz,dz,dma) !(nx1,ny,nz1)

	call interpx_cf(ymu,nx,ny1,nz,amb) !(nx1,ny1,nz)
	call interpz_cf(ymu,nx,ny1,nz,dz,dmb) !(nx,ny1,nz1)
	...
	do j = 1, ny
	do i = 1, nx
		dmd(i,j,k)=mu(i,j,k) /dzz(k)
		dma(i,j,k)=dma(i,j,k)/dz(k)
		dmb(i,j,k)=dmb(i,j,k)/dz(k)
	enddo
	enddo
	enddo
	...
	do i = 1, nx1
		...
		Fu(i,j,k) = -uint(i,j,k) + ((ama(i,j,k)*2.*(u(i+1,j,k)-u(i,j,k))-&
                            ...
                            dma(i,j,k+1)*((u(i,j,k+1)-u(i,j,k))/dz(k)+(w(i,j,k+1)-w(i-1,j,k+1))/dx) - &
                            dma(i,j,k)*  ((u(i,j,k)-u(i,j,k-1))/dz(k)+(w(i,j,k)  -w(i-1,j,k))/dx  ))/rhox(i,j,k)
	enddo
	enddo	
	...
	do i = 1, nx
		...
		Fv(i,j,k) = -vint(i,j,k) + (amb(i+1,j,k)*((v(i+1,j,k)-v(i,j,k))/dx+(u(i+1,j,k)-u(i+1,j-1,k))/dy) - &
                            ...
                            dmb(i,j,k+1)*((v(i,j,k+1)-v(i,j,k))/dz(k)+(w(i,j,k+1)-w(i,j-1,k+1))/dy ) - &
                            dmb(i,j,k)  *((v(i,j,k)-v(i,j,k-1))/dz(k)+(w(i,j,k)  -w(i,j-1,k))/dy   ))/rhoy(i,j,k)
	enddo
	enddo
	enddo
	...
	do i = 1, nx
		...
		Fw(i,j,k) = -wint(i,j,k) + (amd(i+1,j,k)*((w(i+1,j,k)-w(i,j,k))/dx+(u(i+1,j,k)-u(i+1,j,k-1))/dz(k)) - &
                            		    amd(i,j,k)  *((w(i,j,k)-w(i-1,j,k))/dx+(u(i,j,k)  -u(i,j,k-1))/dz(k)  ) + &
                            		    bmd(i,j+1,k)*((w(i,j+1,k)-w(i,j,k))/dy+(v(i,j+1,k)-v(i,j+1,k-1))/dz(k)) - &
                            		    bmd(i,j,k)  *((w(i,j,k)-w(i,j-1,k))/dy+(v(i,j,k)  -v(i,j,k-1))/dz(k)  ) + &
                            (dmd(i,j,k)*2.*(w(i,j,k+1)-w(i,j,k))-&
                            dmd(i,j,k-1)*2.*(w(i,j,k)-w(i,j,k-1)))/dzz(k))/rhoz(i,j,k)
	enddo
	enddo
	enddo
...
SUBROUTINE convdiff_les(Fka) ! Termos convectivos e difusivos da energia cinética
	...
	call interpz_cf(muaux,nx,ny,nz,dz,dm)
	...
	do i = 1, nx
		Fka(i,j,k) = - adv(i,j,k) + &
			      ...
			      (dm(i,j,k+1)*(ka(i,j,k+1)-ka(i,j,k))-dm(i,j,k)*(ka(i,j,k)-ka(i,j,k-1)))/(dz(k)*dz(k))) /rho(i,j,k)
	enddo
	enddo
	enddo
